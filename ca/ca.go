...
func (ca *CAImpl) CompleteOrder(order *core.Order) {
	...
	csr := order.ParsedCSR
	cert, err := ca.newCertificate(append(parsedCSR.DNSNames, parsedCSR.Subject.CommonName), csr.IPAddresses, csr.PublicKey, order.AccountID, order.NotBefore, order.NotAfter)
	if err != nil {
